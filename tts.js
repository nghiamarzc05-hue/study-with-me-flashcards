// tts.js
// TTS chỉ đọc "word" (tiếng Anh). Ưu tiên giọng en-US nếu có.
...
export function speakText(tts, text){
  ...
  if (tts.voiceURI){
    ...
    const vUS = voices.find(x => (x.lang || "").toLowerCase().startsWith("en-us"));
    const vEn = voices.find(x => (x.lang || "").toLowerCase().startsWith("en"));
    if (vUS) u.voice = vUS;
    else if (vEn) u.voice = vEn;
  }
...
export function initTtsUI(tts, { btnSpeak, voiceSelect, rateSelect, onSpeakWord }){
  ...
  function fillVoices(){
    ...
    optAuto.value = "";
    optAuto.textContent = "Tự chọn (ưu tiên US)";
